...
@Component
public class AutoRoleListCommand extends BotCommand {
	...
	public void init() {
		this.name = "listar";
		this.help = "Lista as regras de cargos automáticos";
		this.aliases = new String[] { "list", "lista" };
		this.childOnly = true;
	...
	protected void zapExecute(CommandEvent event) {
		try {
			...
			CommandUtils.splitArgs(Utils.clear(event.getArgs()), 0, name);
			...
			if (autoRoleName.getAutoRoles().getAutoRoles().size() > 0) {
				...
				autoRoleName.getAutoRoles().getAutoRoles().forEach(i -> builder.addField(i.getId().getEmoji(), i.getRole(), false));
