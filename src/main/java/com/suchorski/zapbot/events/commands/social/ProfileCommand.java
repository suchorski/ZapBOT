	...
	protected void zapExecute(CommandEvent event) {
		try {
			...
			if (!member.getUser().isBot()) {
				...
				event.getChannel().sendFile(SocialProfileGenerator.generate(botMember, member.getEffectiveName(), member.getUser().getName(), member.getUser().getDiscriminator(), member.getUser().getEffectiveAvatarUrl(), member.getOnlineStatus(), member.getColor()), String.format("%s.jpg", member.getUser().getName())).queue(m -> {
