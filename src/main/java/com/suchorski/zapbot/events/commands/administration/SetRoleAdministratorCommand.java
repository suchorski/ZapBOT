...
@Component
public class SetRoleAdministratorCommand extends BotCommand {
	...
	public void init() {
		...
		this.aliases = new String[] { "administrator", "admin" };
	...
	protected void zapExecute(CommandEvent event) {
		event.getMessage().getMentionedRoles().stream().findFirst().ifPresentOrElse(r -> {
			try {
				...
				rolesService.setAdministratorRole(roles, r.getName());
