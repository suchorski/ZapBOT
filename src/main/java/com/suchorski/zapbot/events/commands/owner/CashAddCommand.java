...
@Component
public class CashAddCommand extends BotCommand {
	...
	public void init() {
		this.name = "adicionar";
		this.help = "Adiciona moedas a um usuário";
		...
		try {
			...
			event.getMessage().getMentionedUsers().stream().findFirst().ifPresentOrElse(u -> {				
				try {
					...
					coinsService.addCoins(coins, quantity);
					log.infof("%d %s adicionadas ao %s", quantity, quantity != 1 ? "moedas" : "moeda", u.getAsMention());
					CommandUtils.success(event, String.format("%d %s adicionadas", quantity, quantity != 1 ? "moedas" : "moeda"));
