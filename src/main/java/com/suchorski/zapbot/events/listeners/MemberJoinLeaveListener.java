...
public class MemberJoinLeaveListener extends ListenerAdapter {
	...
	public void onGuildMemberJoin(GuildMemberJoinEvent event) {
		...
		if (member.getGuild().getAutoRoles().getAutoRole() != null && !event.getUser().isBot()) {
	...
	public void onGuildMemberRemove(GuildMemberRemoveEvent event) {
		memberService.deleteById(new BotMemberID(event.getUser().getIdLong(), event.getGuild().getIdLong()));
	}
